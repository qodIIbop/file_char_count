...
void end_of_word()
{
	if((actual_char==' ' || actual_char=='\n') && isalnum(previous_char))
    {
		...
		{
           	printf("wordletternum=%d\n",wordletternum);
		}
        lettersumarrayfiller();
		if(DEBUG || DEBUG_FILE_STAT)
		{
           	printf("letternumsum[%d]=%d\n",i,letternumsum[i]);
		}
     }
}
...
//	int letter_indent=0,word_indent=0,space_indent=0,nl_indent=0;  //needed for var character indent (if possible)
	//getting the correct number of indents of numbers to be aligned
	if(letter>100)
	{
	...
	{
		printf("The number of letters: %3d\n",letter);
	}

	if(word>100)
	{
    	printf("The number of letters: %7d\n",word);
	}
